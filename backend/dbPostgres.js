...
const READ_COLUMNS = {
  ...
  products: "id, merchant_id, name, description, specifications, category, price, stock, CASE WHEN COALESCE(image, '') LIKE 'data:%' THEN NULL ELSE image END AS image, CASE WHEN COALESCE(images::text, '') LIKE '%data:%' THEN '[]'::jsonb ELSE images END AS images, status, featured, rating, reviews, created_at, updated_at",

          ...
          sections.filter((section) => section && section.label && section.value).length > 0
            ? `<div style="border:1px solid #e5e7eb;border-radius:16px;padding:16px;background:#f9fafb;">
                ${sections
                  ...
                  .map(
                    (section) => `
                      <div style="display:flex;justify-content:space-between;gap:12px;padding:8px 0;border-bottom:1px solid #e5e7eb;">
                        <span style="font-size:12px;font-weight:700;color:#6b7280;text-transform:uppercase;letter-spacing:0.06em;">${section.label}</span>
                        <span style="font-size:14px;font-weight:700;color:#111827;text-align:right;">${section.value}</span>
                      </div>
                    `
                  )
                  .join('')}
